...
Deno.test("ClaudeLlm.setSystemPrompt: 外部からシステムプロンプトを変更できること", async () => {
  const { llm, calls } = createMockedLlm(
  ...
  await collectAll(llm.chat("hello"));
  const system1 = calls[0].system;
  if (Array.isArray(system1)) {
    assertEquals(system1[0].text, "初期プロンプト");
  }

  llm.setSystemPrompt("変更後プロンプト");
  await collectAll(llm.chat("hello"));
  const system2 = calls[1].system;
  if (Array.isArray(system2)) {
    assertEquals(system2[0].text, "変更後プロンプト");
  }
});
